...
- [ ] Read `.mnemos/project.dna.json`
- [ ] Read `.mnemos/agent_context.json`
- [ ] Skim `.mnemos/context/graphs.md` if the task is architectural
...
1. **DNA:** `.mnemos/project.dna.json` → `.mnemos/agent_context.json`
2. **MCP:** `get_dna`, `search`, `impact_analysis`, `list_domains`, `list_flows` — before repo-wide grep
3. **Graphs:** `.mnemos/context/graphs.md`, `architecture.md`, `domains.md`
