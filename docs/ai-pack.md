...
**Version:** `1.0.0`  
**Schema:** `https://MNESTIS.dev/schemas/ai-pack/v1.json`  
**Stability:** v1 is frozen for the entire MNESTIS 1.x line. Additive fields may appear in minor releases; breaking changes require v2.

## What it is

The AI Pack is built once by `@mnestis/core` (`buildAiPack`) and consumed everywhere:

| Consumer | How |
|----------|-----|
| Dashboard | `/json/:repoId` route + AI cockpit |
| CLI | `MNESTIS pack [path]` |
| Dev server | `GET /api/json/:repoId` (Vite workspace) |
| Memory server | `GET /copilot/pack/:repoId` (`MNESTIS serve`) |
...
1. Run `npx MNESTIS .` in the repo root.
2. **Option A:** `@`-mention `.MNESTIS/project.dna.json` in chat.
3. **Option B:** Paste output of `npx MNESTIS pack --section=summary --mode=ai`.
4. **Option C:** Point the agent at `http://localhost:4000/copilot/pack/local` (with `MNESTIS serve` running).
...
```
X-MNESTIS-AiPack-Version: 1.0.0
X-MNESTIS-AiPack-Schema: https://MNESTIS.dev/schemas/ai-pack/v1.json
```
