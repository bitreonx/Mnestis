# MNESTIS Architecture

MNESTIS transforms a codebase into structured intelligence through a local-first pipeline.
...
Written to `.MNESTIS/` after `MNESTIS build`:
...
Run `npx MNESTIS .` on any repo to generate the full set under `.MNESTIS/context/`.
...
4. **MCP + REST** — `MNESTISRuntime` shared by `MNESTIS serve` and `MNESTIS mcp`
...
| `@mnestis/cli` | `MNESTIS` command-line interface |
